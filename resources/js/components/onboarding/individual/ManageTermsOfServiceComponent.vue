<template>
    <div class="container">
        <div class="row justify-content-center">
            ...
            <div id="manage-terms-of-service-container"></div>
        ...
        async mounted() {
                ...
                const openIndividual = () => adyenKycHandler
                    .create('acceptTermsOfServiceComponent', {
                        ...
                        onAccept: (submittedData) => {mountTosComponent()}, // Upon form completion
                        modalView: false, // Show in a modal window or inline
                        onClose: () => {mountTosComponent()} // Optional, when the user selects Close. For the modal view only
                    })
                .mount('#manage-terms-of-service-container'); // Mount to the container you created

                const mountTosComponent = () => {
        ...
        methods: {
            async getSdkToken() {
                ...
                const response = await fetch('/adyen-onboarding-sdk-token',{
                    ...
                    body: '{ "policy": { "roles": ["acceptTermsOfServiceComponent","manageTermsOfServiceComponent"] } }'
                });
